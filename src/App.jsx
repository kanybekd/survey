...
export default class App extends Component {
  ...
  nextPage = ()=>{
    if(this.state.currentPage<this.state.data.length){
  ...
  render() {
    ...
    let disableNext = this.state.currentPage < this.state.data.length ? "" : "true"
    // let disable = !this.state.currentPage ? "true" : ""
    // console.log("data><><",this.state.data[this.state.currentPage ? 1 : 0])
    const datum = [...[this.state.data[this.state.currentPage === this.state.data.length ? this.state.data.length-1 : this.state.currentPage ]]]
    console.log("datum",datum)
    let surveyPages= datum.map(firstpage=>{
        return(
    ...
    let comp = this.state.currentPage < this.state.data.length ? surveyPages : <div id="finalPage">Thanks for submitting</div>
    return (
      <div className="App">
          {comp}
          <div id="pagination">
